...
class Rental
  ...
  def options_flows(days)
    ...
    daily_flow_to_drivy = OPTIONS_DAILY_FLOWS.select { |k, v| @options.include?(k) && v[:beneficiary] == "drivy" }
    {
      owner: daily_flow_to_owner.empty? ? 0 : daily_flow_to_owner.values.map { |v| v[:daily_price] }.reduce(:+) * days,
      drivy: daily_flow_to_drivy.empty? ? 0 : daily_flow_to_drivy.values.map { |v| v[:daily_price] }.reduce(:+) * days
    }
  end
end
