...
OPTIONS_DAILY_FLOWS = {
  ...
  "additional_insurance" => {daily_price: 1000, beneficiary: "drivy"}
}
################################
...
class Rental
  ...
  def options_flows(days)
    daily_flows_to_owner = OPTIONS_DAILY_FLOWS.select{ |k, v| @options.include?(k) && v[:beneficiary] == "owner"}
    daily_flows_to_drivy = OPTIONS_DAILY_FLOWS.select{ |k, v| @options.include?(k) && v[:beneficiary] == "drivy"}
    {
      owner: daily_flows_to_owner.empty? ? 0 : daily_flows_to_owner.values.map{ |v| v[:daily_price] }.reduce(:+) * days,
      drivy: daily_flows_to_drivy.empty? ? 0 : daily_flows_to_drivy.values.map{ |v| v[:daily_price] }.reduce(:+) * days
    }
  end
end
